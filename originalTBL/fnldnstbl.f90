...
DOUBLE PRECISION        :: lamda,utau1,utau2,kp,kk,eps,lamda03,lamda1,x0 
...
DOUBLE PRECISION, dimension(:), allocatable      :: theta,deltastar,Ht,zrecy,new,eta,Reth,Rex
DOUBLE PRECISION, dimension(:,:,:,:), allocatable :: Utold  ! 3d Flow Field (t:i:j:k)
integer :: threevar,ksep,cnt2
...
!These variables are internal to statistics 
 ...
 !do k=1,n3d(3)
  ...
  !read(12,97)x1(i),x2(j),x3(k),Ut(i,j,k,1),Ut(i,j,k,2),Ut(i,j,k,3),Ut(i,j,k,4),Press(i,j,k)
   !                  end do 
   !            end do
   !         end do
...
!             do j=1,n3d(2)
!                  do k=1,n3d(3)
!Ut(i,j,k,2)=Ut(i,j,k,2)*3.0d0
!Ut(i,j,k,3)=Ut(i,j,k,3)*3.0d0
!                     end do 
!                end do
!            end do
x0=32.0d0
...
!*********************************Reading velocity at inflow from file******************!!

                 Ume(1:n3d(1),1:n3d(2),1:n3d(3),1:3)= 0.0d0

   open (unit=15,file='U_innerdns.dat')
         do k=1,37
...
! then extending the free-stream velocity to all higher points

   do k=38,n3d(3)
     Ume(1,1,k,1) = Ume(1,1,37,1)
   enddo
...
! Copy the array to all points 
  ...
  do k=1,n3d(3)
     do j=1,n3d(2)
      do i=1,n3d(1)
                     Ut(i,j,k,1)= Ume(i,j,k,1)+(0.01d0*Ue*dsin(beta*x2(j))*dcos(gama*x3(k)))
                     Ut(i,j,k,2)=              (0.01d0*Ue*dsin(beta*x2(j))*dcos(gama*x3(k)))
                     Ut(i,j,k,3)=              (0.01d0*Ue*dsin(beta*x2(j))*dcos(gama*x3(k)))
         enddo
     enddo
      enddo
     enddo
...
if(k.le.2.or.k.ge.(n3d(3)-1)) then
  ...
  divuu(3)= ((Ut(i,j,k-2,3)-8*Ut(i,j,k-1,3)+8*Ut(i,j,k+1,3)-Ut(i,j,k+2,3))*(onetwelfth*invdXi))
    ...
    if (my_rank.eq.0) write(*,21)time,nstep,sqrt(abs(diver)/((n3d(2)-2)*(n3d(1)-2)*(n3d(3)-2)))
        do i=1,5
         UbDNS(i) =0.0d0
     itemp= iend
...
sumxz(:) = sumxz(:)/1000.0d0

     open(2,file='numrcltauwall.xy',status='unknown',access='append')
        do i=2,n3d(1)-1
         write(2,94)time, x1(i), sumxz(i)/dble((n3d(2)-2)),2.0d0*(sumxz(i)/(Ue*Ue*dble(n3d(2)-2))),Rex(i)
        enddo
      close(2)
...
!if (my_rank.eq.0) write (12,03)'title =""'
if (my_rank.eq.0) write (12,03)'TITLE ="',time,nstep,'"'
if (my_rank.eq.0) write (12,*)'Variables = "x","y","z","U","V","W","T","P"'
...
!      do i=2,n3d(1)-1
      do k=istart,iend
    write(12,97)x1(i),x2(j),x3(k),Ut(i,j,k,1),Ut(i,j,k,2),Ut(i,j,k,3),Ut(i,j,k,4),Press(i,j,k)
      end do
      end do
      end do
      close(12)
...
!if(mod(nstep,100).eq.0) then
 open (unit=19,file='boundarylthickness.dat',status='unknown')
       do i=1,n3d(1)
    write(19,433) time,x1(i), bltkns(i), deltastar(i), theta(i), Ht(i), Reth(i),Rex(i)
       enddo
 close(19)
!endif

433 format(8(2x,F20.10))
...
97    format(3(1X,F10.6),5(1X,E22.15))
